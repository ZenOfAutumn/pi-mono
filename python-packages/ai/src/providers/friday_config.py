...
class FridayAuthConfig:
    ...
    def to_headers(self) -> Dict[str, str]:
        ...
        """
        headers = {
            "Authorization": f"Bearer {self.billing_id}",
            "Content-Type": "application/json;charset=UTF-8",
        }

        if self.agent_id:
            headers["Mt-Agent-Id"] = self.agent_id

        if self.context_token:
            headers["Mt-Context-Token"] = self.context_token

        if self.user_id:
            headers["Mt-User-Id"] = self.user_id
...
def build_friday_request_params(
    ...
    # 思考模型配置
    if "reasoning" in options or "thinkTokens" in options:
        think_config: Dict[str, any] = {"enabled": True}
        if "thinkTokens" in options:
